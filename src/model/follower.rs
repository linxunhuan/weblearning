...
use sea_orm::{
    ActiveModelTrait, ColumnTrait, Condition, DbErr, EntityTrait, NotSet, QueryFilter, Value,
};
...
pub async fn find_follower_ids_model(followee: u32) -> Result<Vec<u32>, DbErr> {
    ...
    let uids = wb_follower::Entity::find()
        .filter(Condition::all().add(wb_follower::Column::FolloweeId.eq(followee)))
        .all(db)
        .await?;
...
pub async fn update_follow_model(follower: u32, followee: u32, status: u8) -> Result<(), DbErr> {
    ...
    let _ = wb_follower::Entity::update_many()
        .col_expr(
            wb_follower::Column::Status,
            Expr::value(Value::TinyUnsigned(Some(status))),
        )
        .col_expr(
            wb_follower::Column::UpdatedTime,
            Expr::value(Value::Unsigned(Some(local_timestamp()))),
        )
        .filter(
            Condition::all()
                .add(wb_follower::Column::FollowerId.eq(follower))
