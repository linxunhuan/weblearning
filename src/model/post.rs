...
use sea_orm::{
    ActiveModelTrait, ColumnTrait, Condition, DbBackend, DbErr, EntityTrait, NotSet, QueryFilter,
    Statement, Value,
};
...
pub async fn find_post_model(pid: u32) -> Result<Option<Model>, DbErr> {
    ...
    let post = wb_post::Entity::find()
        .filter(Condition::all().add(wb_post::Column::Pid.eq(pid)))
        .one(db)
        .await?;
    Ok(post)
}
...
pub async fn update_post_model(uid: u32, pid: u32) -> Result<(), DbErr> {
    ...
    let _ = wb_post::Entity::update_many()
        .col_expr(
            wb_post::Column::Status,
            Expr::value(Value::TinyUnsigned(Some(1))),
        )
        .col_expr(
            wb_post::Column::UpdatedTime,
